...
import com.wultra.security.powerauth.lib.nextstep.model.entity.CredentialValue;
import com.wultra.security.powerauth.lib.nextstep.model.entity.enumeration.EncryptionAlgorithm;
 ...
 */
@Component
public class CredentialValueConverter {

    private static final Logger logger = LoggerFactory.getLogger(CredentialValueConverter.class);
     ...
     */
    @Autowired
    public CredentialValueConverter(NextStepServerConfiguration configuration) {
    ...
    /**
     * Convert credential value to database value with optional record encryption.
     *
     * @param credentialValue Credential value.
     * @param userId User ID.
     * @param credentialDefinition Credential definition.
     ...
     */
    public CredentialValue toDBValue(String credentialValue, String userId, CredentialDefinitionEntity credentialDefinition) throws InvalidConfigurationException, EncryptionException {
        if (!credentialDefinition.isEncryptionEnabled()) {
        ...
        switch (credentialDefinition.getEncryptionAlgorithm()) {
            ...
            case AES_HMAC -> {
                ...
                try {
                    ...
                    final byte[] encrypted = aes.encrypt(credentialValue.getBytes(StandardCharsets.UTF_8), iv, secretKey);
    ...
    /**
     * Derive secret key from master DB encryption key, user ID and credential name.
     *
     ...
     * @throws InvalidConfigurationException Thrown when Next Step configuration is invalid.
     */
    private SecretKey deriveSecretKey(SecretKey masterDbEncryptionKey, String userId, String credentialName) throws InvalidConfigurationException {
        try {
            // Use concatenated user ID and credential name bytes as index for KDF_INTERNAL
            final byte[] index = (userId + "&" + credentialName).getBytes(StandardCharsets.UTF_8);
