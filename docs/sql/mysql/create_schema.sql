...
CREATE TABLE ns_application (
  ...
  name                   VARCHAR(255) NOT NULL,                        -- Application name used for identification.
  description            VARCHAR(255),                                 -- Description of the application.
  status                 INTEGER NOT NULL,                             -- Application status: ACTIVE, REMOVED.
  organization_id        VARCHAR(255),                                 -- Organization this application belongs to.
...
CREATE TABLE ns_credential_policy (
  ...
  name                       VARCHAR(255) NOT NULL,                        -- Credential policy name used for identification.
  description                VARCHAR(255),                                 -- Description of the credential policy.
  status                     INTEGER NOT NULL,                             -- Credential policy status: ACTIVE, REMOVED.
  username_length_min        INTEGER,                                      -- Minimum length of username.
  username_length_max        INTEGER,                                      -- Maximum length of username.
  username_allowed_chars     VARCHAR(255),                                 -- Allowed characters in username (regular expression).
  ...
  credential_allowed_chars   VARCHAR(255),                                 -- Allowed characters in credential value.
  ...
  username_gen_algorithm     VARCHAR(255) DEFAULT 'DEFAULT' NOT NULL,      -- Algorithm used for generating the username.
  credential_gen_algorithm   VARCHAR(255) DEFAULT 'DEFAULT' NOT NULL,      -- Algorithm used for generating the credential.
...
CREATE TABLE ns_user_identity (
  user_id                VARCHAR(255) NOT NULL PRIMARY KEY,           -- User identity identifier (not autogenerated).
  status                 INTEGER NOT NULL,                            -- User identity status: ACTIVE, BLOCKED, REMOVED.
  extras                 VARCHAR(255),                                -- Extra attributes with data related to user identity.
...
CREATE TABLE ns_user_contact (
  ...
  user_id                VARCHAR(255) NOT NULL,                       -- User identity identifier.
  name                   VARCHAR(255) NOT NULL,                       -- User contact name used for identification.
  type                   INTEGER,                                     -- User contact type: PHONE, EMAIL, OTHER.
  value                  VARCHAR(255),                                -- User contact value.
...
CREATE TABLE ns_user_identity_history (
  ...
  user_id                  VARCHAR(255) NOT NULL,                       -- User identity identifier.
  status                   INTEGER NOT NULL,                            -- User identity status: ACTIVE, BLOCKED, REMOVED.
  roles                    VARCHAR(255),                                -- Assigned user roles.
  extras                   VARCHAR(255),                                -- Extra attributes with data related to user identity.
...
CREATE TABLE ns_role (
  ...
  name                   VARCHAR(255) NOT NULL,                       -- Role name used for identification.
...
CREATE TABLE ns_user_role (
  ...
  status                   INTEGER NOT NULL,                            -- User role status: ACTIVE, BLOCKED, REMOVED.
...
CREATE TABLE ns_user_alias (
  ...
  user_id                  VARCHAR(255) NOT NULL,                       -- User identity identifier.
  name                     VARCHAR(255) NOT NULL,                       -- User alias name used for identification.
  value                    VARCHAR(255) NOT NULL,                       -- User alias value.
  status                   INTEGER NOT NULL,                            -- User alias status: ACTIVE, REMOVED.
  extras                   VARCHAR(255),                                -- Extra attributes with data related to user alias.
...
CREATE TABLE ns_hashing_config (
  ...
  name                     VARCHAR(255) NOT NULL,                       -- Hashing configuration name used for identification.
  algorithm                VARCHAR(255) NOT NULL,                       -- Hashing algorithm name.
  status                   INTEGER NOT NULL,                            -- Hashing configuration status: ACTIVE, REMOVED.
  parameters               VARCHAR(255),                                -- Hashing algorithm parameters.
...
CREATE TABLE ns_credential_definition (
  ...
  name                     VARCHAR(255) NOT NULL,                       -- Credential definition name used for identification.
  description              VARCHAR(255),                                -- Description of the credential definition.
  application_id           INTEGER NOT NULL,                            -- Application identifier.
  credential_policy_id     INTEGER NOT NULL,                            -- Credential policy identifier.
  category                 INTEGER NOT NULL,                            -- Credential category: PASSWORD, PIN, OTHER.
  encryption_enabled       INTEGER DEFAULT 0 NOT NULL,                  -- Whether encryption of stored credentials is enabled.
  encryption_algorithm     VARCHAR(255),                                -- Algorithm used for stored credential encryption.
  ...
  status                   INTEGER NOT NULL,                            -- Credential definition status: ACTIVE, REMOVED.
...
CREATE TABLE ns_otp_definition (
  ...
  name                     VARCHAR(255) NOT NULL,                       -- One time password definition name used for identification.
  description              VARCHAR(255),                                -- Description of the one time password definition.
  application_id           INTEGER NOT NULL,                            -- Application identifier.
  ...
  encryption_algorithm     VARCHAR(255),                                -- Algorithm used for stored one time password encryption.
  status                   INTEGER NOT NULL,                            -- One time password definition status: ACTIVE, REMOVED.
...
CREATE TABLE ns_credential_storage (
  ...
  user_id                          VARCHAR(255) NOT NULL,               -- User identity identifier.
  type                             INTEGER,                             -- Credential type: PERMANENT, TEMPORARY.
  user_name                        VARCHAR(255),                        -- Username.
  value                            VARCHAR(255) NOT NULL,               -- Credential value.
  status                           INTEGER NOT NULL,                    -- Credential status: ACTIVE, BLOCKED_TEMPORARY, BLOCKED_PERMANENT, REMOVED.
...
CREATE TABLE ns_otp_storage (
  ...
  value                       VARCHAR(255) NOT NULL,               -- One time password value.
  salt                        VARBINARY(16) NOT NULL,              -- Cryptographic salt used when generating one time password.
  status                      INTEGER NOT NULL,                    -- One time password status: ACTIVE, USED, BLOCKED, REMOVED.
...
CREATE TABLE ns_authentication (
  authentication_id           INTEGER NOT NULL PRIMARY KEY AUTO_INCREMENT, -- Authentication identifier (autogenerated).
  user_id                     VARCHAR(255),                                -- User identity identifier.
  type                        INTEGER NOT NULL,                            -- Authentication type: CREDENTIAL, OTP, CREDENTIAL_OTP.
  credential_id               VARCHAR(255),                                -- Credential identifier.
  otp_id                      VARCHAR(255),                                -- One time password identifier.
  operation_id                VARCHAR(256),                                -- Operation identifier.
  result                      INTEGER NOT NULL,                            -- Overall authentication result.
  result_credential           INTEGER,                                     -- Authentication result for credential authentication.
  result_otp                  INTEGER,                                     -- Authentication result for one time password authentication.
...
CREATE TABLE ns_operation_history (
  ...
  authentication_id           INTEGER,                             -- Reference to the authentication record.
...
CREATE TABLE ns_audit_log (
  ...
  action                 VARCHAR(255) NOT NULL,                       -- Action which is being audited.
...
CREATE INDEX ns_credential_storage_lookup ON ns_credential_storage (credential_definition_id, user_name);
CREATE UNIQUE INDEX ns_credential_storage_unique ON ns_credential_storage (user_id, credential_definition_id, user_name);
