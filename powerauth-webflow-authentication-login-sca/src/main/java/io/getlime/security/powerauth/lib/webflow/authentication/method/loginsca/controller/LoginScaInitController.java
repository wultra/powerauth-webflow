...
public class LoginScaInitController extends AuthMethodController<LoginScaInitRequest, LoginScaInitResponse, AuthStepException> {
    ...
    public PrepareLoginFormDataResponse prepareLoginForm(@RequestBody PrepareLoginFormDataRequest request) throws AuthStepException {
        if (request == null) {
        ...
        if (operation.getUserId() != null && operation.getOrganizationId() != null) {
            ...
            response.setMobileTokenEnabled(mobileTokenEnabled);
            logger.info("Prepare login form data skipped, user and organization is already known");
            return response;
        }
        try {
